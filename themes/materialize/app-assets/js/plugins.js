(function ($, Drupal, drupalSettings) {
  Drupal.behaviors.plugins = {
    attach: function attach(context) {
      // Globally letiables
      let sidenavMain = $(".sidenav-main");
      let navCollapsible = $(".navbar .nav-collapsible");
      let breadcrumbsWrapper = $("#breadcrumbs-wrapper");
      ...
      function navigationCollapse() {
         if (!$(".sidenav-main.nav-collapsible").hasClass("nav-lock")) {
            let openLength = $(".collapsible .open").children().length;
            ...
            setTimeout(function () {
               ...
               if (openLength > 1) {
                  let collapseEl = $(".sidenav-main .collapsible");
                  let collapseInstance = M.Collapsible.getInstance(collapseEl);
      ...
      $(function () {
         ...
         let indeterminateCheckbox = document.getElementById("indeterminate-checkbox");
         ...
         $(".dropdown-submenu").on("mouseenter", function () {
            ...
            let dd = $this.find('.dropdown-content');
            if (dd) {
               let ddLeft = dd.offset().left,
                  ddWidth = dd.width();
         ...
         let allCollapse = document.querySelectorAll(".collapsible");
         ...
         if (breadcrumbsWrapper.attr("data-image")) {
            let imageUrl = breadcrumbsWrapper.attr("data-image");
         ...
         // Main Menu (SideNav)
         //------------------
         let collapsibleSubCollapsible = $("li.active .collapsible-sub .collapsible");
         let sidemainCollapse = document.querySelectorAll(".sidenav-main .collapsible");
         ...
         if ($("body").hasClass("menu-collapse")) {
            let sidenavCollapse = $(".sidenav-main .collapsible");
         ...
         $(".nav-collapsible .navbar-toggler").once('nav-collapsible-toggle').click(function () {
         ...
         if (!is_touch_device()) {
            ...
            if ($("#slide-out.leftside-navigation").length > 0) {
               if (!$("#slide-out.leftside-navigation").hasClass("native-scroll")) {
                  let ps_leftside_nav = new PerfectScrollbar(".leftside-navigation", {
                     wheelSpeed: 2,
            ...
            if ($(".slide-out-right-body").length > 0) {
               let ps_slideout_right = new PerfectScrollbar(".slide-out-right-body, .chat-body .collection", {
                  suppressScrollX: true,
                  wheelPropagation: false
               });
            }
            if ($(".chat-body .collection").length > 0) {
               let ps_slideout_chat = new PerfectScrollbar(".chat-body .collection", {
                  suppressScrollX: true
               });
            }
            // for horizonatal nav scroll
            if ($("#ul-horizontal-nav").length > 0) {
               let ps_horizontal_nav = new PerfectScrollbar("#ul-horizontal-nav", {
                  wheelPropagation: false,
               });
            }
         ...
         let chatScrollAuto = $("#right-sidebar-nav #slide-out-chat .chat-body .collection");
         ...
         //    let currentLanguage = $this.find("a").data("language");
