...
class Element extends DrupalAttributes {
  ...
  public function &__get($key) {
    if (\Drupal\Core\Render\Element::property($key)) {
      throw new \InvalidArgumentException('Cannot dynamically retrieve element property. Please use \Drupal\materialize\Utility\Element::getProperty instead.');
    }
  ...
  public function __set($key, $value) {
    if (\Drupal\Core\Render\Element::property($key)) {
      throw new \InvalidArgumentException('Cannot dynamically retrieve element property. Use \Drupal\materialize\Utility\Element::setProperty instead.');
    }
  ...
  public function __isset($name) {
    if (\Drupal\Core\Render\Element::property($name)) {
      throw new \InvalidArgumentException('Cannot dynamically check if an element has a property. Use \Drupal\materialize\Utility\Element::unsetProperty instead.');
    }
  ...
  public function __unset($name) {
    if (\Drupal\Core\Render\Element::property($name)) {
      throw new \InvalidArgumentException('Cannot dynamically unset an element property. Use \Drupal\materialize\Utility\Element::hasProperty instead.');
    }
    parent::__unset($name);
  }
  ...
  /**
   * Adds a specific Materialize class to color a button based on its text value.
   *
   * @return $this
   */
  public function colorize() {
    ...
    $class = $button && !Materialize::getTheme()->getSetting('button_colorize') ? 'btn-default' : FALSE;
    ...
    if (!$class) {
      ...
      $class = "$prefix-" . Materialize::cssClassFromString($value, $button ? $this->getProperty('button_type', 'default') : 'default');
    }
  ...
  /**
   * Creates a new \Drupal\materialize\Utility\Element instance.
   *
   * @param array|string $element
  ...
  /**
   * Creates a new standalone \Drupal\materialize\Utility\Element instance.
   *
   ...
   */
  public static function findChildByName(&$elements, $name) {
    $results = [];
    $children = \Drupal\Core\Render\Element::children($elements, FALSE);
    foreach ($children as $key) {
      ...
      if (is_array($child)) {
        if ($key == $name) {
          ...
          $results = Element::findChildByName($child, $name);
        }
      }
      if ($results) {
        break;
  ...
  /**
   * Adds Materialize button size class to the element.
   *
   * @param string $class
  ...
  public function setButtonSize($class = NULL) {
    ...
    if (!isset($classes)) {
      $classes = [];
      if ($button_size = Materialize::getTheme()->getSettingPlugin('button_size')) {
    ...
    if (!$class) {
      $class = Materialize::getTheme()->getSetting('button_size');
    }
   ...
   * @return $this
   *
   * @see \Drupal\materialize\Materialize::glyphicon()
   */
  public function setIcon(array $icon = NULL) {
    if ($this->isButton() && !Materialize::getTheme()->getSetting('button_iconize')) {
      return $this;
    }
    if ($value = $this->getProperty('value', $this->getProperty('title'))) {
      $icon = isset($icon) ? $icon : Materialize::iconFromString($value);
