...
class Theme {
   ...
   *
   * @see \Drupal\materialize\Theme::IGNORE_ASSETS
   * @see \Drupal\materialize\Theme::IGNORE_CORE
   * @see \Drupal\materialize\Theme::IGNORE_DOCS
   * @see \Drupal\materialize\Theme::IGNORE_DEV
   */
  ...
  /**
   * Flag indicating if the theme is Materialize based.
   *
   * @var bool
   */
   ...
   *
   * @var \Drupal\materialize\Plugin\UpdateManager
   */
  ...
  public function __construct(Extension $theme, ThemeHandlerInterface $theme_handler) {
    ...
    $this->materialize = $this->subthemeOf('materialize');

    // Only install the theme if it's Materialize based and there are no schemas
    // currently set.
    if ($this->isMaterialize() && !$this->getSetting('schemas')) {
      try {
        $this->install();
      }
      catch (\Exception $e) {
  ...
  public function __wakeup() {
    $theme_handler = Materialize::getThemeHandler();
  ...
  public function drupalSettings() {
    // Immediately return if theme is not Materialize based.
    if (!$this->isMaterialize()) {
      return [];
    }
   ...
   *     - \Drupal\materialize::IGNORE_CORE
   *     - \Drupal\materialize::IGNORE_ASSETS
   *     - \Drupal\materialize::IGNORE_DOCS
   *     - \Drupal\materialize::IGNORE_DEV
   *     - \Drupal\materialize::IGNORE_THEME
  ...
  public function fileScan($mask, $subdir = NULL, array $options = []) {
    ...
    if (!isset($options['nomask']) && $flags) {
      ...
      if ($flags & self::IGNORE_DEV) {
        $ignore_directories += ['bower_components', 'grunt', 'node_modules', 'STARTERKIT'];
      }
    ...
    if (!$files->has($hash)) {
      $files->set($hash, \Drupal::service('file_system')->scanDirectory($path, $mask, $options));
    }
   ...
   *   An associative array of \Drupal\materialize objects (theme), keyed
   *   by machine name.
   */
  ...
  public function getPendingUpdates() {
    ...
    // Only continue if the theme is Materialize based.
    if ($this->isMaterialize()) {
   ...
   *
   * @return \Drupal\materialize\Plugin\Provider\ProviderInterface|false
   ...
   *
   * @return \Drupal\materialize\Plugin\Provider\ProviderInterface[]
   ...
   *
   * @return \Drupal\materialize\Plugin\UpdateManager|false
   *   The Update plugin manager or FALSE if theme is not Materialize based.
   */
  ...
  /**
   * Determines whether or not if the theme has Materialize Framework Glyphicons.
   */
  ...
  protected function install() {
    ...
    $schemas = [];
    foreach ($this->getAncestry() as $ancestor) {
      // todo: $schemas[$ancestor->getName()] = $ancestor->getUpdateManager()->getLatestSchema();
    }
    $this->setSetting('schemas', $schemas);
  }

  /**
   * Indicates whether the theme is materialize based.
   *
   * @return bool
   *   TRUE or FALSE
   */
  public function isMaterialize() {
   ...
   *   TRUE or FALSE
   *
   * @see \Drupal\materialize\Theme::dev
   */
   ...
   *   The livereload URL.
   *
   * @see \Drupal\materialize\Theme::livereload
   */
