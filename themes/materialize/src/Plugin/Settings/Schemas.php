...
namespace Drupal\materialize\Plugin\Setting;

use Drupal\materialize\Materialize;
use Drupal\materialize\Plugin\Form\SystemThemeSettings;
...
class Schemas extends SettingBase {
  ...
  public function alterFormElement(Element $form, FormStateInterface $form_state, $form_id = NULL) {
    ...
    // todo: handling updates - figure out, add.
    /*
    $updates = [];
    ...
    '#icon' => Materialize::glyphicon('open'),
    ...
    if ($theme = SystemThemeSettings::getTheme($form, $form_state)) {
      ...
      if ($operations) {
        ...
        batch_set([
          ...
          'file' => Materialize::autoloadFixInclude(),
        ]);
      }
    }
  }
  ...
  public static function batchProcessUpdate($theme_name, $update_id, array &$context) {
    ...
    $theme = Materialize::getTheme($theme_name);

    // Reconstruct the update plugin that is being applied.
    list($provider, $plugin_id) = explode(':', $update_id);
    $provider = Materialize::getTheme($provider);

    /** @type \Drupal\materialize\Plugin\Update\UpdateInterface $update */
   ...
   *   \Drupal\materialize\Plugin\Setting\Update::batchProcess().
   * @param $operations
   *   If $success is FALSE, contains the operations that remained unprocessed.
   */
  public static function batchFinished($success, $results, $operations) {
    /** @type \Drupal\materialize\Theme $theme */
    // Reconstruct the theme object this update is being applied to.
    $theme = Materialize::getTheme($results['theme_name']);
