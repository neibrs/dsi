...
class MergeContactsAction extends ConfigurableActionBase implements ContainerFactoryPluginInterface {
   ...
   *   The renderer service.
   */
  public function __construct(array $configuration, $plugin_id, $plugin_definition, ModuleHandler $module_handler, TranslationManager $translation_manager, EntityTypeManager $entity_type_manager, EntityFieldManager $entity_field_manager, Renderer $renderer) {
  ...
  public function executeMultiple(array $objects) {
    ...
    foreach (array_keys($wrappers) as $contact_id) {
      ...
      $this->entityTypeManager->getStorage('path_alias')->save('/crm-core/contact/' . $primary_contact->id(), '/crm-core/contact/' . $contact_id);
      if ($this->moduleHandler->moduleExists('crm_core_activity')) {
        // Replacing participant in existing activities.
        $query = $this->entityTypeManager->getStorage('crm_core_activity')->getQuery();
      ...
      if ($this->moduleHandler->moduleExists('relation')) {
        ...
        $query = $this->entityTypeManager->getStorage('relation')->getQuery();
