...
class EntityContentBase extends EntityContentBaseBase {
  ...
  public function import(Row $row, array $old_destination_id_values = []) {
    ...
    foreach ($field_definitions as $field_name => $field_definition) {
      // 只通过 migration 配置处理基础字段，不自动匹配基础字段.
    ...
    // 根据 import.settings 取 default_value_fields. 该方法今后要取消.
    if (empty($default_value_fields)) {
      $default_value_fields = \Drupal::config('import.settings')->get('default_value_fields');
      $default_value_fields = isset($default_value_fields[$entity_type_id]) ? $default_value_fields[$entity_type_id] : [];
    }

    if (!empty($default_value_fields)) {
      // 根据用户在导入表单设置的 default_value_fields 设置默认值.
      ...
      $empty_destination_properties = array_diff($empty_destination_properties, $default_value_fields);
